...
function LevelViewCtrl($sce, $state, $stateParams, CurrentUserService, LevelService, LevelHintService, LevelValidationService, LoggerService, Restangular) {
  ...
  var loadBoughtLevel = function () {
    // Fetches status of the level for current organization
    LevelService.getOrganizationLevel(currentOrg, vm.level).then(function (orgLevel) {
      if (orgLevel._id) {
  ...
  var validate = function (validation) {
    LevelValidationService.validateOrganizationLevel(validation, vm.orgLevel).then(
    //SUCCESS
    function (data) {
  ...
  var requestLevelInstance = function (instance) {
    ...
    return Restangular.all("level-instance-users").post({
      ...
      return Restangular.one("level-instance-users", response.data._id).get().then(function (userLevelInstance) {
        var user = CurrentUserService.getUser();
        var hash = userLevelInstance.data.hash;
        var parts = instance.urls[0].url.replace("http://", "").replace("/", "").split(":");
  ...
  var selectInstance = function (instance) {
...
angular.module("portal.levels").controller("LevelViewCtrl", LevelViewCtrl);