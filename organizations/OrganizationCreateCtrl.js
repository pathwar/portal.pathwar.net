...
function OrganizationCreateCtrl($q, $state, CurrentUserService, SessionService, OrganizationService, Restangular, LoggerService) {
  /** Retrieve user sessions */
  var init = function () {
    var user = CurrentUserService.getUser();

    $q.all([SessionService.getPublicSessions(), OrganizationService.getOrganizationsByUserId(user._id)]).then(function (results) {
  ...
  var create = function (formData) {
    ...
    OrganizationService.create(org).then(function (organization) {
      //TODO: This is DIRTY
      CurrentUserService.switchOrganization(organization).then(CurrentUserService.loadUserInfo).then(function () {
...
angular.module("portal.organizations").controller("OrganizationCreateCtrl", OrganizationCreateCtrl);